...
WorkBuddy Skin Manager 是面向非技术用户的本地桌面工具。选择一张背景图，应用会在本机提取配色、生成主题并自动应用；之后无论 WorkBuddy 刷新还是重启，都会自动恢复你的主题。
...
- 我的主题：预览、应用、删除保存在本机的主题。
- 常驻守护：WorkBuddy 刷新、重开或重启后自动重新应用主题。
- 版本与位置检测：显示检测到的 WorkBuddy 版本和安装路径。
- 一键恢复：恢复官方外观并停止主题守护。

## 用户流程

1. 打开 WorkBuddy Skin Manager，侧栏底部会显示检测到的 WorkBuddy 版本。
2. 进入「从图片生成」，选择 PNG、JPEG 或 WebP 背景图并填写主题名称。
3. 应用在本机分析配色、保存主题并自动应用。
...
macOS 会检查 `/Applications/WorkBuddy.app` 和 `~/Applications/WorkBuddy.app`；Windows 会依次检查正在运行的 WorkBuddy、卸载注册表和常见安装目录。自定义安装目录未被识别时，可将 `WORKBUDDY_PATH` 环境变量设置为 WorkBuddy 应用或 `WorkBuddy.exe` 的完整路径。
...
- 取色与图片处理全部在本机完成，不上传。
- 主题状态使用原子写入、备份与故障恢复；应用失败后自动恢复普通启动模式。
...
| `npm test` | 运行 theme engine 单元测试 |
| `npm run check` | 前端构建 + theme engine 测试 |
