...
1. 确认 WorkBuddy 版本符合测试主题 manifest 中的兼容范围，并正常启动。
...
7. 重新加载 WorkBuddy 页面，等待最多 60 秒，确认守护任务重新应用主题。
...
12. 导入一个背景图片接近 2 MB 或更大的 `.wbskin`，确认不仅颜色生效，主内容区背景也真实显示；主题引擎结果中的 `backgroundPresent` 必须为 `true`，`backgroundImage` 不能为 `none` 或原始 Base64 data URL。
